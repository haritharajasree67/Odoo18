...
class AccountBankBookReport(models.TransientModel):
    ...
    account_ids = fields.Many2many(
        'account.account', 'account_account_bankbook_report',
    ...
    @api.model
    def get_bank_book_data(self, params):
        """
        Called via RPC from the OWL BankBook component.
        Accepts a single dict `params` so it works cleanly with
        /web/dataset/call_kw positional args.

        params keys:
            date_from, date_to, target_move, sortby,
            display_account, initial_balance,
            journal_ids (list of ints),
            account_ids (list of ints)
        """
        try:
            ...
            # Build context — mirrors _build_comparison_context
            ctx = {
            ...
            # Delegate to existing report model
            report = self.env['report.om_account_daily_reports.report_bankbook']
            ...
            _logger.exception("get_bank_book_data failed — params: %s", params)
            raise