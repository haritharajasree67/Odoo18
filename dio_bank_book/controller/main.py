...
class BankBookController(http.Controller):
    ...
    def _get_records(self, env, params):
        ...
        if not accounts:
            return []
        report = env['report.om_account_daily_reports.report_bankbook']
    ...
    # ── JSON preview ──────────────────────────────────────────────────────────
    @http.route('/bank_book/get_data', type='json', auth='user', methods=['POST'])
    def get_bank_book_data(self, **kwargs):
        try:
            ...
            _logger.exception("BankBookController.get_bank_book_data failed")
            return {'status': 'error', 'data': [], 'message': 'Server error — check Odoo logs'}

    # ── PDF download ──────────────────────────────────────────────────────────
    @http.route('/bank_book/get_pdf', type='json', auth='user', methods=['POST'])
    def get_bank_book_pdf(self, **kwargs):
        try:
            params = self._resolve_params(kwargs)
            env    = request.env

            # Step 1: fetch the same data we show on screen
            ...
            # Step 2: build the data dict for our own QWeb template.
            # _get_report_values() will receive this as `data`.
            report_data = {
                'accounts':       accounts,
            ...
            # Step 3: get the report_name string from our ir.actions.report record
            # In Odoo 18, _render_qweb_pdf is called on env['ir.actions.report']
            # with report_ref = report_name string as the FIRST positional argument
            report_name = 'dio_bank_book.report_dio_bank_book'
            ...
            # Step 5: save attachment and return download URL
            date_label = (params['date_from'] or 'all').replace('-', '')
            filename   = f"bank_book_{date_label}.pdf"
            ...
            _logger.exception("BankBookController.get_bank_book_pdf failed")
            return {'status': 'error', 'message': 'PDF generation failed — check Odoo logs'}

    # ── XLSX export ───────────────────────────────────────────────────────────
    @http.route('/bank_book/get_xlsx', type='json', auth='user', methods=['POST'])
    def get_bank_book_xlsx(self, **kwargs):
        try:
            import xlsxwriter
            ...
            worksheet = workbook.add_worksheet('Bank Book')

            fmt_title    = workbook.add_format({'bold': True, 'font_size': 14, 'font_color': '#1a1a2e'})
            fmt_header   = workbook.add_format({'bold': True, 'bg_color': '#1a1a2e', 'font_color': '#ffffff',
                                                'border': 1, 'align': 'center', 'valign': 'vcenter'})
            fmt_account  = workbook.add_format({'bold': True, 'bg_color': '#e8eaf6', 'border': 1, 'font_size': 11})
            fmt_num_acc  = workbook.add_format({'bold': True, 'bg_color': '#e8eaf6', 'border': 1,
            ...
            worksheet.merge_range(0, 0, 0, 8,
                f"Account Bank Book   {date_from_label} — {date_to_label}", fmt_title)
            worksheet.set_row(0, 22)

            for col, h in enumerate(['Date','JRNL','Partner','Ref','Move','Entry Label','Debit','Credit','Balance']):
                worksheet.write(1, col, h, fmt_header)
            worksheet.set_row(1, 18)

            row = 2
            for account in data:
                worksheet.merge_range(row, 0, row, 5, f"{account['code']} — {account['name']}", fmt_account)
                ...
                for line in account['move_lines']:
                    ...
                    worksheet.write(row, 8, line['balance'],      pf if line['balance'] >= 0 else ngf)
                    row += 1

            workbook.close()
            filename   = f"bank_book_{(params['date_from'] or 'all').replace('-','')}.xlsx"
            ...
            return {'status': 'ok', 'url': f'/web/content/{attachment.id}?download=true', 'filename': filename}

        except Exception:
            _logger.exception("BankBookController.get_bank_book_xlsx failed")
